...
void ImportSchedule(ifstream &fin,FileSchedule *&s,int &countcsv,ifstream &fin2,Stu *&a,int & NumofStu)
{
	...
	{
		cout << "Cant open file";
		cout << "Please enter file Schedule Name again: " << endl;
		getline(cin, z);
		fin.open(z.c_str());
	}
	...
	LoadStudentClass(fin2, a, NumofStu);
	fin2.close();

	for (int i = 0; i < countcsv; i++)
	{
		string filename;
		filename = x + "-" + y + "-" + Class + "-" + s[i].courseID + "-Student.txt"; //2019-2020-HK2-19CLC5-CS1612-Student.txt
